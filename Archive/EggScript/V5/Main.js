...
class EggScript {
    ...
    if (request) {
      ...
      switch (kind) {
        ...
        case "deleteSheet":
          ...
          break;
        case "addChart":
          requestArray[requestArray.length - 1].addChart = request;
          break;
        case "updateChartSpec":
          requestArray[requestArray.length - 1].updateChartSpec = request;
          break;
        case "updateEmbeddedObjectPosition":
          requestArray[requestArray.length - 1].updateEmbeddedObjectPosition = request;
          break;
        default:
  ...
  static rowsMinusCountblank(sheet, column){ 
    return `ROWS('${sheet}'!${column}:${column})-COUNTBLANK('${sheet}'!${column}:${column})`;
  }
...
// Function for debugging and such
function testZone() {
  let requestArray = [];
  let spreadsheet = EggScript.getSpreadsheet();
  // Add a new object sheet, apparently it doesn't work like that though
  if (0) {
    let addSheetRequest = Sheets.newAddSheetRequest();
    addSheetRequest.properties = Sheets.newSheetProperties();
    addSheetRequest.properties.title = "stock-price-chart";
    addSheetRequest.properties.sheetType = "OBJECT";
    EggScript.requestArrayPush(requestArray, "addSheet", addSheetRequest);
  }
  // Add a new sheet without a grid
  if (0) {
    let addSheetRequest = Sheets.newAddSheetRequest();
    addSheetRequest.properties = Sheets.newSheetProperties();
    addSheetRequest.properties.title = "stock-price-chart";
    addSheetRequest.properties.gridProperties = Sheets.newGridProperties();
    addSheetRequest.properties.gridProperties.hideGridlines = true;
    EggScript.requestArrayPush(requestArray, "addSheet", addSheetRequest);
  }
  // Add the chart as its own sheet
  if (0) {
    let addChartRequest = Sheets.newAddChartRequest();
    addChartRequest.chart = Sheets.newEmbeddedChart();
    addChartRequest.chart.spec = Sheets.newChartSpec();
    addChartRequest.chart.spec.title = "stock-price-chart";
    addChartRequest.chart.spec.maximized = true;
    addChartRequest.chart.spec.basicChart = Sheets.newBasicChartSpec();
    addChartRequest.chart.spec.basicChart.chartType = "LINE";
    ...
    addChartRequest.chart.spec.basicChart.axis[0].viewWindowOptions = Sheets.newChartAxisViewWindowOptions();
    addChartRequest.chart.spec.basicChart.axis[1].position = "BOTTOM_AXIS";
    addChartRequest.chart.spec.basicChart.axis[1].viewWindowOptions = Sheets.newChartAxisViewWindowOptions();
    // addChartRequest.chart.spec.basicChart.domains = [];
    ...
    EggScript.requestArrayPush(requestArray, "addChart", addChartRequest);
  }
  // Get the test chart
  if (0) {
    let sheet = EggScript.getSheetFromTitle(spreadsheet, "stock-price-chart");
    Logger.log(sheet.charts[0]);
  }
  // Put an existing chart on a new sheet
  if (0) {
    let sheet = EggScript.getSheetFromTitle(spreadsheet, "test-zone-chart");
    let chart = EggScript.getChartFromTitle(sheet, "test-zone-chart");
    let ueopRequest = Sheets.newUpdateEmbeddedObjectPositionRequest();
    ueopRequest.objectId = chart.chartId;
    ueopRequest.newPosition = Sheets.newEmbeddedObjectPosition();
    ueopRequest.newPosition.newSheet = true;
    EggScript.requestArrayPush(requestArray, "updateEmbeddedObjectPosition", ueopRequest);
  }
  // Try to get specific values from the chart
  if (0) {
    let stockPriceSheet = EggScript.getSheetFromTitle(spreadsheet, "stock-price");
    let stockPriceChartSheet = EggScript.getSheetFromTitle(spreadsheet, "stock-price-chart");
    let stockPriceChart = EggScript.getChartFromTitle(stockPriceChartSheet, "stock-price-chart");
    Logger.log(stockPriceChart.spec.basicChart.series);
  }
  // Update the stock price chart, maybe
  if (0) {
    EggScript.requestArrayPush(requestArray, "updateChartSpec", 
      updateStockPriceChartRequest(spreadsheet));
  }
  // formSubmissionStack length
  if (0) {
    let extraDataSheet = EggScript.getSheetFromTitle(spreadsheet, "extra-data");
    Logger.log(EggScript.getDimensionLength(extraDataSheet, "D"));
  }
  // Object info
  if (1) {
    let extraDataSheet = EggScript.getSheetFromTitle(spreadsheet, "extra-data");
    Logger.log(extraDataSheet.data[0].getRowData()[0].values[3].toString() === "{}");
  }
  // Logger.log(requestArray);
  EggScript.batchUpdate(requestArray);
}
// Add a series to stock-price-chart representing the new column in stock-price
function updateStockPriceChartRequest(spreadsheet) {
  let stockPriceSheet = EggScript.getSheetFromTitle(spreadsheet, "stock-price");
  let stockPriceChartSheet = EggScript.getSheetFromTitle(spreadsheet, "stock-price-chart");
  let stockPriceChart = EggScript.getChartFromTitle(stockPriceChartSheet, "stock-price-chart");
  if (!stockPriceChart.spec.basicChart.series) {
  ...
  if (EggScript.getDimensionLength(stockPriceSheet, 1) > stockPriceChart.spec.basicChart.series.length) {
    ...
    let stockPriceRangeColumn = 
      EggScript.fromDimensionIndex(EggScript.getDimensionLength(stockPriceSheet, 1) - 1, "COLUMNS");
    let stockPriceRange = `${stockPriceRangeColumn}1:${stockPriceRangeColumn}1000`;
    basicChartSeries.series.sourceRange.sources = 
      EggScript.newGridRange(stockPriceSheet.properties.sheetId, stockPriceRange);
    basicChartSeries.targetAxis = "LEFT_AXIS";
    updateChartSpecRequest.spec.basicChart.series.push(basicChartSeries);
