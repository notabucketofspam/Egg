...
namespace EggScriptNamespace {
  ...
  export class EggScript {
     ...
     * @param spreadsheetId
     */
    static getSpreadsheet(spreadsheetId: string) {
     ...
     * @param dimension
     */
    static toDimensionIndex(dimension: any) {
     ...
     */
    static fromDimensionIndex(dimensionIndex: number, majorDimension: string) {
      // Taken from the same source
      ++dimensionIndex;
      let dimension: number | string;
     ...
     * @param range
     */
    static newGridRange(sheetId: number, range: string) {
     ...
     */
    static newRowData() {
      let values: GoogleAppsScript.Sheets.Schema.CellData[] = [];
      return { values };
    }
    /**
     ...
     * @param value
     */
    static newCellData(value: number | string | boolean) {
     ...
     * @param value
     */
    static newExtendedValue(value: number | string | boolean) {
     ...
     * @param range
     */
    static newUpdateCellsRequest(rows: GoogleAppsScript.Sheets.Schema.RowData[], fields: string,
     ...
     * @param dimension
     */
    static getDimensionLength(sheet: GoogleAppsScript.Sheets.Schema.Sheet, dimension: any) {
     ...
     * @param dimension
     */
    static clearDimensionRequest(sheet: GoogleAppsScript.Sheets.Schema.Sheet, dimension: any) {
      let dimensionLength = EggScript.getDimensionLength(sheet, dimension);
      let rows: GoogleAppsScript.Sheets.Schema.RowData[] = [];
      if (EggScript.isColumn(dimension)) {
        // Case where the dimension is a column
     ...
     * @param cell
     */
    static clearCellRequest(sheet: GoogleAppsScript.Sheets.Schema.Sheet, cell: string) {
      let rowData = EggScript.newRowData();
      let rows = [rowData];
      let fields = "userEnteredValue";
     ...
     * @param title
     */
    static validateSheetRequestArray(spreadsheet: GoogleAppsScript.Sheets.Schema.Spreadsheet, title: string) {
        ...
        EggScript.calculationTitleArray.forEach(function(calculationTitle) {
          ...
          if (calculationTitle === "stock-price-initial") {
            let sheetColumnPriceListRowData =
              sheet.data[0].rowData && sheet.data[0].rowData[1] ?
     ...
     * @param spreadsheetId
     */
    static batchUpdate(requestArray: GoogleAppsScript.Sheets.Schema.Request[], spreadsheetId: string) {
     ...
     * @param title
     */
    static getSheetFromTitle(spreadsheet: GoogleAppsScript.Sheets.Schema.Spreadsheet, title: string) {
     ...
     * @param request
     */
    static requestArrayPush(requestArray: GoogleAppsScript.Sheets.Schema.Request[], kind: string, request: any) {
      if (request) {
     ...
     * @param cell
     */
    static calculationRequest(calculation: string, sheet: GoogleAppsScript.Sheets.Schema.Sheet, cell: string) {
      let rowData = EggScript.newRowData();
     ...
     * @param row
     */
    static indirectConcatenate(sheetTitle: string, column: string, row: string) {
     ...
     */
    static formulaCalculationRequestArray(spreadsheet: GoogleAppsScript.Sheets.Schema.Spreadsheet,
     ...
     * @param formSheetTitle
     */
    static undoCalculationRequestArray(spreadsheet: GoogleAppsScript.Sheets.Schema.Spreadsheet,
      formSheetTitle: string) {
      let requestArray: GoogleAppsScript.Sheets.Schema.Request[] = [];
     ...
     * @param title
     */
    static getChartFromTitle(sheet: GoogleAppsScript.Sheets.Schema.Sheet, title: string) {
      let titleArray: string[] = [];
     ...
     * @param spreadsheet
     */
    static updateStockPriceChartRequest(spreadsheet: GoogleAppsScript.Sheets.Schema.Spreadsheet) {
