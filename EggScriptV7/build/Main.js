...
function processFormSubmission(form) {
    ...
    let formSubmissionGridRange = EggScript.newGridRange(formSubmissionSheet.properties.sheetId, "A" + formSheetNewSubmissionRowNumber + ":C" + formSheetNewSubmissionRowNumber);
    EggScript.requestArrayPush(requestArray, "updateCells", EggScript.newUpdateCellsRequest(formSubmissionRows, formSubmissionFields, formSubmissionGridRange));
    ...
    let formSubmissionStackGridRange = EggScript.newGridRange(extraDataSheet.properties.sheetId, "D" + (extraDataSheet.data[0].rowData[0].values[3].toString() === "{}" ? 1 :
        EggScript.getDimensionLength(extraDataSheet, "D") + 1));
    EggScript.requestArrayPush(requestArray, "updateCells", EggScript.newUpdateCellsRequest(formSubmissionStackRows, formSubmissionStackFields, formSubmissionStackGridRange));
    ...
    Array.prototype.push.apply(requestArray, EggScript.formulaCalculationRequestArray(spreadsheet, form["sheet-select"]));
...
function processFormUndo() {
    ...
    EggScript.requestArrayPush(requestArray, "updateCells", EggScript.newUpdateCellsRequest(gaffeCounterRows, gaffeCounterFields, gaffeCounterGridRange));
    // Pop an entry from formSubmissionStack
    let formSubmissionStackLength = EggScript.getDimensionLength(extraDataSheet, "D");
    EggScript.requestArrayPush(requestArray, "updateCells", EggScript.clearCellRequest(extraDataSheet, "D" + formSubmissionStackLength));
    // Clear the last row of the relevant sheet
    let formSubmissionStackLastFormTitle = extraDataSheet.data[0].rowData[formSubmissionStackLength - 1].values[3].userEnteredValue.stringValue;
    let formSubmissionStackLastFormSheet = EggScript.getSheetFromTitle(spreadsheet, formSubmissionStackLastFormTitle);
    EggScript.requestArrayPush(requestArray, "updateCells", EggScript.clearDimensionRequest(formSubmissionStackLastFormSheet, EggScript.getDimensionLength(formSubmissionStackLastFormSheet, "A")));
    // Pop an entry from the relevant column in each of the calculation sheets
    Array.prototype.push.apply(requestArray, EggScript.undoCalculationRequestArray(spreadsheet, formSubmissionStackLastFormTitle));
    // Update the spreadsheet
    EggScript.batchUpdate(requestArray, spreadsheetId);
    return gaffeCounter;
}
        ...
        switch (sheet.properties.title) {
            ...
            case "stock-price":
                ...
                for (let index = 0; index < EggScript.getDimensionLength(sheet, "1"); ++index) {
                    EggScript.requestArrayPush(requestArray, "updateCells", EggScript.clearDimensionRequest(sheet, EggScript.fromDimensionIndex(index, "COLUMNS")));
                }
                break;
            default:
